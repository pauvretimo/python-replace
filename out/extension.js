...
function activate(context) {
    ...
    const scriptPath = path.resolve(__dirname, "../src/python_scripts/replace_str.py");
    // get os lunux/unix or windows
    const os = process.platform;
    // if windows use "py" else use "python3"
    const python = os == "win32" ? "py" : "python3";
    // get
    let python_replace_str = vscode.commands.registerCommand("python-replace.replace", () => {
        ...
        vscode.window.showInputBox({ prompt: "match string" }).then((value) => {
            ...
            vscode.window.showInputBox({
                ...
                (0, child_process_1.exec)(String.raw `${python} "${scriptPath}" "${file_path}" "${match_str}" "${replace_fun}"`, (error, stdout, stderr) => {
                    if (error) {
